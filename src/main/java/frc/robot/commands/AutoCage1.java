...
public class AutoCage1 extends Command {
   ...
   */
  public AutoCage1(SwerveDrive m_Swerve, Arm m_Arm ,Lift m_Lift, Wrist m_Wrist, Intake m_Intake) {
  ...
  public void execute() {
    if(timer.get()<1){
      ...
      m_Lift.liftOut();
      m_Swerve.diagonalAngle(1.3, -2.8);//angulo
    }else if(timer.get() > 4.6 && timer.get() < 6.6){
      m_Arm.armOut();
    }else if(timer.get() > 6.7 && timer.get() < 8.3 ){
      m_Lift.goToFloor();
      m_Swerve.diagonalOdometri(1.3, -2.8);//distancia
